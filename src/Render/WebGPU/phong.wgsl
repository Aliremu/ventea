...
fn main(input: VSIn) -> VSOut {
    ...
    let world_pos: vec4<f32> = transform[input.instance_id] * vec4<f32>(input.position + normalize(input.normal) / 1000.0, 1.0);
...
fn main(input: FSIn) -> @location(0) vec4<f32> {
    ...
    let view_dir = normalize(cam_pos - input.frag_pos);
    ...
    for (var i: u32 = 0u; i < 3u; i = i + 1u) {
       //Lo += max(dot(normal, normalize(light_positions[i] - input.frag_pos)), 0.0) * albedo;
        let light = lights[i];
        let light_dir = normalize(input.frag_pos - light.position);
        if light.light == 0.0 {
            Lo += light.color * pow(dot(input.normal, -light_dir) * 0.5 + 0.5, 2.0);

        }
    }
    
    return vec4(Lo, sample.a);
}